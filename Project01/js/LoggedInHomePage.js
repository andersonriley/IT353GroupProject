...
function displayMovie() {
  ...
  $.get(url, data).done(function(response){
    $("#output")
    .append("<h1>Movie</h1><img width=250 alt='Movie Image Unavailable' src=" + response.Poster + "/><p>Title:<p class=indent>" + response.Title + "</p><p>Description:<p class=indent>" + response.Plot + "</p>")
  })
  .fail(function(jqXHR){
...
function displayBook() {
  ...
  $.get(url, data).done(function(response){
    $("#output").append("<h1>Book</h1><img width=250 alt='Book Image Unavailable' src=" + response.items[0].volumeInfo.imageLinks.smallThumbnail + "/><br><p>Title: "+ response.items[0].volumeInfo.title +"</p><p>Description: "+ response.items[0].volumeInfo.description +"</p>")
  })
  .fail(function(jqXHR){
...
function displayPic() {
  ...
  $.get(url, data).done(function(response){
    $("#output").append("<h1>Image</h1><img width=250 alt='Picture Function Image' src=" + response.hits[0].largeImageURL + "/><br><p>Tags: "+ response.hits[0].tags +"</p>")
  })
  .fail(function(jqXHR){
